...
export class ProviderInboxPage implements OnInit, OnDestroy {
  ...
  readonly filteredConversations = computed(() => {
    ...
    return this.conversations().filter(conversation => {
      ...
      // Si no hay término de búsqueda, siempre mostrar
      const matchesTerm = !term || name.includes(term) || preview.includes(term);
      // El filtro 'all' siempre muestra, 'unread' solo si hay no leídos
      const hasUnread = (conversation.unreadCount ?? 0) > 0;
      const matchesFilter = filter === 'all' ? true : hasUnread;
      // Siempre mostrar si hay id, aunque no haya mensajes ni displayName
      return (conversation.id != null) && matchesTerm && matchesFilter;
    });
  });
  ...
  refreshConversations(event?: RefresherCustomEvent, options: { silent?: boolean } = {}): void {
    ...
    this.chatService
      ...
      .subscribe({
        next: conversations => {
          // console.log('[ProviderInbox] Conversaciones recibidas:', conversations);
        ...
        error: error => {
          console.error('ProviderInboxPage: failed to load conversations', error);
          if (!silent) {
  ...
  openConversation(conversation: ConversationUI): void {
    this.router.navigate(['/provider/chat', conversation.id]);
  }
  ...
  private setupRealtimeSync(): void {
    ...
    if (!this.authService.isTokenValid()) {
      console.warn('ProviderInboxPage: token inválido o expirado, omitiendo conexión WS');
      return;
    }
    ...
    this.webSocketService.getAuthExpired$()
      .pipe(takeUntil(this.destroy$))
      .subscribe(() => {
        console.warn('ProviderInboxPage: sesión expirada detectada por WS, redirigiendo a login');
    ...
    this.webSocketService.connectToNotifications().catch(error =>
      console.error('ProviderInboxPage: error connecting to notifications socket', error)
    );
