...
@Component({
  selector: 'app-provider-chats',
  templateUrl: './provider-chats.page.html',
  styleUrls: ['./provider-chats.page.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule, IonicModule]
})
export class ProviderChatsPage implements OnInit, OnDestroy {
  ...
  refreshConversations(event?: RefresherCustomEvent, options: { silent?: boolean } = {}): void {
    ...
    this.chatService
      ...
      .subscribe({
        ...
        error: error => {
          console.error('ProviderChatsPage: failed to load conversations', error);
          if (!silent) {
  ...
  private setupRealtimeSync(): void {
    if (!this.authService.isTokenValid()) {
      console.warn('ProviderChatsPage: token inválido o expirado, omitiendo conexión WS');
      return;
    }
    ...
    this.webSocketService.getAuthExpired$()
      .pipe(takeUntil(this.destroy$))
      .subscribe(() => {
        console.warn('ProviderChatsPage: sesión expirada detectada por WS, redirigiendo a login');
    ...
    this.webSocketService.connectToNotifications().catch(error =>
      console.error('ProviderChatsPage: error connecting to notifications socket', error)
    );
